...
def signup():
    ...
    if request.method == "POST":
        ...
        if acnt:
            ...
            create_table = 'CREATE TABLE {}(prodname VARCHAR(20),quantity integer,warehouse_location varchar(20),date varchar(15))'.format(str(username))
...
def add_stock():
    ...
    if request.method == "POST":
        ...
        if prodname=="" or quantity=="" or date=="":
            ...
            ibm_db.bind_param(pstmt,4,date)
            ibm_db.execute(pstmt)
...
def delete_stock():
    ...
    if(request.method=="POST"):
        ...
        if prodname=="":
                ...
                return render_template('delete_stock.html',msg=msg)
...
def dashboard():
    ...
    if request.method == "POST":
        ...
        if prodname=="" or quantity=="" :
            ...
            Updated_stock=acnt['QUANTITY']-int(quantity)
            if Updated_stock>0:
                pass
            else:
                alert("Please update the quantity of the product {}, Atleast {} number of pieces must be added!".format(prodname,10))
            ...
            insert_sql='UPDATE {} SET  quantity=?,warehouse_location=? WHERE prodname=? '.format(session['username'])
            pstmt=ibm_db.prepare(conn, insert_sql)
            ibm_db.bind_param(pstmt,1,Updated_stock)
            ibm_db.bind_param(pstmt,2,warehouse_location)
            ibm_db.bind_param(pstmt,3,prodname)
            ibm_db.execute(pstmt)
            msg='You have successfully updated the products!!'
